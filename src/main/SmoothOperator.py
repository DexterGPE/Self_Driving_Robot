...
class SmoothOperator:
    ...
    def self_driving(cls, bp, speed_left, speed_right, pars):
        ...
        try:
            if cls.bumped_into_wall(bp):
                ...
                speed_left, speed_right = cls.smooth_left_turn_on_bridge(speed_left,
                                                                         speed_right,
                                                                         pars)
            elif cls.get_right_wall_distance(bp) > 23:
                speed_left, speed_right = cls.smooth_right_turn_on_bridge(speed_left,
                                                                          speed_right,
                                                                          pars)
            else:
    ...
    def smooth_turn_at_wall(cls, bp, pars):
        ...
        correction_factor = max(-1, min(1, (distance - pars["distance_to_wall"]) / pars[
            "smoothness"]))
