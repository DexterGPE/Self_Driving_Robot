...
class DifferentMapLayout(SmoothOperator):
    ...
    def smooth_right_turn_on_bridge(speed_left, speed_right, pars):
        speed_left = max(pars["standard_speed"] + pars["turn_speed"] * pars["turn_factor"],
                         speed_left + (pars["turn_speed"] / (pars["bridgesmoothness"] * pars["smoothness"])))
        speed_left = min(speed_left, pars["standard_speed"] - pars["turn_speed"] * pars["turn_factor"])

        speed_right = min(pars["standard_speed"] - pars["turn_speed"] * pars["turn_factor"],
                          speed_right - (pars["turn_speed"] / (pars["bridgesmoothness"] * pars["smoothness"])))
        speed_right = min(speed_right, pars["standard_speed"] - pars["turn_speed"] * pars["turn_factor"])
